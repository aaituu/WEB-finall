...
document.addEventListener('DOMContentLoaded', function() {
    if (window.location.pathname.includes('profile.html')) {
        ...
        document.getElementById('clearHistoryBtn')?.addEventListener('click', function() {
            if (confirm('Are you sure you want to delete all feedback history? This action cannot be undone.')) {
...
function loadProfileData(userId) {
    ...
    document.getElementById('feedbackCount').textContent = user.feedback?.length || 0;
            ...
            <div class="card border-start border-primary border-4">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        <div>
                            <h5 class="card-title mb-1">${feedback.subject || 'General Inquiry'}</h5>
...
function filterFeedback(userId) {
    ...
    // Filter by subject
    if (subjectFilter) {
        filteredFeedback = filteredFeedback.filter(f => f.subject === subjectFilter);
    }
